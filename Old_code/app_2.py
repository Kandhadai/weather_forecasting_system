...
# Example route for Alerts - requires login
@app.route('/alerts', methods=['GET', 'POST'])
def manage_alerts():
    if not session.get('username'):
        return login_required()  # Redirect to login if not authenticated
    ...
    advisories = control.check_health_advisory(session.get('user_id'), user_location)
    return render_template('alerts.html', alerts=alerts, advisories=advisories)


# Similarly, apply the same check to Feedback, Dashboard, Data Export, etc.
...
# Route for FAQ (accessible without login)
@app.route('/faq')
def faq():
    return render_template('faq.html')

# Route for Educational Content (accessible without login)
...
def view_weather():
    ...
    # For GET request, just render the form without data
